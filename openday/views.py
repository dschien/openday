...
from openday.forms import ContactForm
from openday.models import Survey, Selection, Contact
import logging
import datetime
...
def start(request, group=None):
    ...
    if not request.session:
        ...
        return render(request, 'start.html', {'error_message': "Your session had time out. Start again.", }, )

    logger.info('<start> : {}'.format(request.session.session_key))
    return render(request, 'start.html', {'group': group})
...
def gender(request, group=None):
    ...
    if request.POST['answer'] == 'Skip survey':
        ...
        return HttpResponseRedirect(reverse('app', kwargs={'group': group}))
...
def climate(request, group=None):
    ...
    if len(errors) > 0:
        return render('gender.html', {'error_message': 'Please answer each question.'}, )
...
def rank(request, group=None):
    ...
    if len(errors) > 0:
        return render(request, 'climate.html', {k: True for k in errors}, )
    ...
    if request.session['type'] == 'survey':
        ...
        if not re.search('Skip', request.POST['answer']):
            if not 'rank' in request.POST or not 'confidence' in request.POST:
                return render(request, 'rank.html', {'error_message': 'Please make all selections'},
                              )
...
def app(request, group=None):
    ...
    if request.session['type'] == 'survey':
        ...
        if not re.search('Skip', request.POST['answer']):
            if not 'rate' in request.POST \
                ...
                return render(request, 'rate.html', {'error_message': 'Please answer each question.'},
                              )
    ...
    return render(request, 'index.html', {'type': request.session['type']}, )
...
def prereview(request, group=None):
    ...
    logger.info('<prereview> sid: {} , POST:{}'.format(request.session.session_key, request.POST))
    return render(request, 'prereview.html', {'type': request.session['type']},
                  )
...
def review(request, group=None):
    ...
    if not re.search('Skip', request.POST['answer']):
        if not 'opinion_change' in request.POST:
            return render(request, 'prereview.html', {'error_message': 'Please choose one answer'},
                          )
    ...
    return render(request, 'review_no_change.html', {}, )
...
def thankyou(request, group=None):
    ...
    if not re.search('Skip', request.POST['answer']):
        ...
        if not 'expect' in request.POST \
            ...
            if s.opinion_change:
                return render(request, 'review.html', {'error_message': 'Please choose one answer'},
                              )
            return render(request, 'review_no_change.html', {'error_message': 'Please choose one answer'},
                          )
...
def createSelections(jsonArray):
    ...
    for json in jsonArray:
        s = Selection(time=json['duration'], connection=json['connection'], device=get_device(json),
...
def export(request):
    ...
    for field in ['age', 'cc', 'cit', 'duration', 'expect', 'gender', 'group', 'home_rank', 'home_rank_confidence',
                  'id', 'it', 'new_rank', 'opinion_change', 'pk', 'rate_confidence', 'rating', 'survey_date', 'ua']:
        fields += field + ","
    ...
    for s in list:
        ...
        html += "{}, {} , {}, {}, {}, {}, {}, {}, {}, {}, {}, {} , {} , {} , {} , {} , {}".format(s.age, s.cc, s.cit,
                                                                                                  s.duration, s.expect,
                                                                                                  s.gender, s.group,
                                                                                                  s.home_rank,
                                                                                                  s.home_rank_confidence,
                                                                                                  s.id, s.it,
                                                                                                  s.new_rank,
                                                                                                  s.opinion_change,
                                                                                                  s.pk,
...
def disclaimer(request, group=None):
    logger.info("view: " + str(stack()[0][3]))
    return render(request, 'start.html', {'group': 'none'})
